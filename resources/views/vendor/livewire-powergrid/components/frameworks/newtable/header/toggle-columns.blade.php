    ...
    <div x-data="{open: false}"
        ...
        <button @click.prevent="open = ! open"
                class="block bg-slate-50 text-slate-700 border border-slate-300 rounded py-1.5 px-3 leading-tight
                       focus:outline-none focus:bg-white focus:border-slate-600 dark:border-slate-500
        ...
        <div x-show="open"
            ...
            @foreach($columns as $column)
                ...
                <div wire:click="$emit('pg:toggleColumn-{{ $tableName }}', '{{ $column->field }}')"
                     ...
                     class="@if($column->hidden) opacity-40 bg-slate-300 dark:bg-slate-800 @endif cursor-pointer flex justify-start block px-4 py-2 text-slate-800 hover:bg-slate-50 hover:text-gray-200 dark:text-slate-900 text-lg dark:hover:bg-gray-500 dark:hover:bg-slate-700">
                    @if(!$column->hidden)
