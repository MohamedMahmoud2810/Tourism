<div x-data="{ open: false }"
    ...
    <button @click.prevent="open = ! open"
            class="block bg-slate-50 text-slate-700   rounded py-1.5 px-3 leading-tight
                   focus:outline-none focus:bg-white  white:border-slate-500 white:bg-slate-700
                   2xl:dark:placeholder-slate-300 dark:text-slate-200 dark:text-slate-300">
        <div class="flex">
            <x-livewire-powergrid::icons.download class="h-10 w-10 text-slate-700 white:text-slate-600"/>
        </div>
    </button>
    ...
    <div x-show="open"
        ...
        @if(in_array('excel', data_get($setUp, 'exportable.type')))
            <div class="flex px-4 py-2 text-slate-400 dark:text-slate-700 text-xl">
                <span class="w-12">@lang('Excel')</span>
                <a x-on:click="$wire.call('exportToXLS'); open = false"
                   href="#"
                   class="px-2 block text-slate-800 hover:bg-slate-50 hover:text-black-300 dark:text-slate-700 text-xl dark:hover:bg-slate-50 rounded">
